	...
	FT_Int      y, SoftSurface *pSoftSurf, glColorBytes fgColor)
{
	...
	for (i = x, p = 0; i < x_max; i++, p++)
	{
		for (j = y, q = 0; j < y_max; j++, q++)
		{
			...
			uint8 bwvalue = bitmap->buffer[q * bitmap->width + p];
...
SurfaceAnim * FreeTypeManager::TextToSurfaceAnim(CL_Vec2f surfaceSizeToCreate, string msg, float pixelHeight,
	glColorBytes bgColor, glColorBytes fgColor, bool bUseActualWidthForSpacing, vector<CL_Vec2f> *pOptionalLineStarts,
	float wordWrapX)
{
	vector<unsigned short> utf16line;
	...
	return TextToSurfaceAnim(surfaceSizeToCreate, utf16line, pixelHeight,
		bgColor, fgColor, bUseActualWidthForSpacing, pOptionalLineStarts, wordWrapX);
}
...
bool FreeTypeManager::TextToSoftSurface(SoftSurface *pSoftSurf, CL_Vec2f surfaceSizeToCreate, vector<unsigned short> utf16line, float pixelHeight,
	glColorBytes bgColor, glColorBytes fgColor, bool bUseActualWidthForSpacing, vector<CL_Vec2f> *pOptionalLineStarts,
	float wordWrapX)
{
	int minSize = 10;
	...
	FT_Bool       use_kerning;
	if (surfaceSizeToCreate.x < minSize) surfaceSizeToCreate.x = minSize;
	...
	float wordWrapX)
{
	...
	if (!TextToSoftSurface(&softSurf, surfaceSizeToCreate, utf16line, pixelHeight, bgColor, fgColor,
		bUseActualWidthForSpacing, pOptionalLineStarts, wordWrapX))
	{
		return NULL;
	}
...
SurfaceAnim * FreeTypeManager::TextToSurfaceAnim(CL_Vec2f surfaceSizeToCreate, vector<unsigned short> utf16line, float pixelHeight,
	glColorBytes bgColor, glColorBytes fgColor, bool bUseActualWidthForSpacing, vector<CL_Vec2f> *pOptionalLineStarts,
	float wordWrapX)
{
	SoftSurface softSurf;
	...
	softSurf.FlipY();
	pSurf->InitFromSoftSurface(&softSurf);
