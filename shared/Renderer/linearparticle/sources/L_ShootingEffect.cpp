...
L_ShootingEffect::L_ShootingEffect(int x, int y, const CL_Vec2f& shooting_vector_t, int period_t, int particle_per_period_t) :
...
void L_ShootingEffect::howto_emit_particle(void)
{
	...
	shooting_angle_cache = linear_get_radian(shooting_vector);
	...
	for(j=0;j<particle_per_period;j++)
	{
		...
		CL_Vec2f v_t = shooting_vector;
		linear_set2(v_t, shooting_magnitude_cache+current_speed_dis, shooting_angle_cache+radian_t);
...
void L_ShootingEffect::set_shooting_vector(const CL_Vec2f& vector)
{
	shooting_vector = vector;

	shooting_magnitude_cache = shooting_vector.length();
	shooting_angle_cache = linear_get_radian(shooting_vector);
}
...
void L_ShootingEffect::set_shooting_vector(L_REAL magnitude, L_REAL radian)
{
	...
	linear_set2(shooting_vector, shooting_magnitude_cache, shooting_angle_cache);
}
...
CL_Vec2f L_ShootingEffect::get_shooting_vector(void)
{
