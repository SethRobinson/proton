...
public:
	...
	L_Color(uint32 rgba)
	{
		r = (unsigned char)GET_RED(rgba);
		g = (unsigned char)GET_GREEN(rgba);
		b = (unsigned char)GET_BLUE(rgba);
		a = (unsigned char)GET_ALPHA(rgba);

	}
	L_Color(unsigned char r,unsigned char g,unsigned char b,unsigned char a)
	{
		this->a = a;
		this->r = r;
		this->g = g;
...
public:
	...
	L_Particle(SurfaceAnim* surface_t, int life_t);
	...
	/** Set blending mode of particle  */
	void set_blending_mode(Surface::eBlendingMode mode);
	...
	void Setup(SurfaceAnim *surf, int life_t);
};
