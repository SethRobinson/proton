...
| `AI/` | LLM chat-completion client for OpenAI-compatible servers, and a text-to-speech client for form-POST TTS servers (see `docs/ai-llm.md`) | `LLMClient.h/.cpp` (LLMConversation + LLMClient; needs `Network/` + `util/cJSON.c` compiled in), `TTSClient.h/.cpp` (text -> audio file; a request pool with a priority queue; needs `Network/`) |
