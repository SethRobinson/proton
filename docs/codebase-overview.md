...
| `Network/` | HTTP and sockets | `NetHTTP.h/.cpp` (+ `NetHTTP_libCURL.cpp`, `html5/NetHTTP_HTML5.cpp`; the socket backend has an opt-in stream mode for SSE, see `docs/ai-llm.md`), `NetSocket.h`, `NetUtils.h`, `NetAdapter/`, vendored `enet/` |
| `AI/` | LLM chat-completion client for OpenAI-compatible servers (whole replies or SSE streaming with the model's reasoning), and a text-to-speech client for form-POST TTS servers (see `docs/ai-llm.md`) | `LLMClient.h/.cpp` (LLMConversation + LLMStreamParser + LLMClient; needs `Network/` + `util/cJSON.c` compiled in), `TTSClient.h/.cpp` (text -> audio file; a request pool with a priority queue; needs `Network/`) |
